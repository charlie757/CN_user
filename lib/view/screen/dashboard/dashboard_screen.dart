...
import 'package:flutter_sixvalley_ecommerce/view/screen/product/new_arrival_product_screen.dart';
import 'package:flutter_sixvalley_ecommerce/view/screen/service/service_screen.dart';
...
class DashBoardScreenState extends State<DashBoardScreen> {
  ...
  void initState() {
    ...
    _screens = [
      const HomePage(),
     const NewArrivalProductScreen(),
     const OrderScreen(isBacButtonExist: false),
      const ServiceScreen(),
      // const NotificationScreen(isBacButtonExist: false),
      const MoreScreen()
      // singleVendor?const OrderScreen(isBacButtonExist: false):
      // const InboxScreen(isBackButtonExist: false) ,
      // singleVendor? const NotificationScreen(isBacButtonExist: false):
      // const OrderScreen(isBacButtonExist: false),
      // singleVendor? const MoreScreen():
      // const NotificationScreen(isBacButtonExist: false),
  ...
  List<BottomNavigationBarItem> _getBottomWidget(bool isSingleVendor) {
    ...
    if(!isSingleVendor){
      ...
      list.add(_barItem(Images.messageImage, getTranslated('product', context), 1));
      list.add(_barItem(Images.shoppingImage, getTranslated('orders', context), 2));
      list.add(_barItem(Images.bag, getTranslated('service', context), 3));
