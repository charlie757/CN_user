...
class LocationProvider with ChangeNotifier {
  ...
  Future<void> initcountryApi() async {
    clearCountryValues();
    ApiResponse apiResponse = await locationRepo!.getCountryList();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<void> initstateyApi(String id) async {
    stateList.clear();
    cityList.clear();
    statedropdownvalue = null;
    citydropdownvalue = null;
    ApiResponse apiResponse = await locationRepo!.getStateList(id);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      apiResponse.response!.data.forEach((element) {
        CountryModel model = CountryModel.fromJson(element);
  ...
  Future<void> initcityApi(String id) async {
    cityList.clear();
    citydropdownvalue = null;
    ApiResponse apiResponse = await locationRepo!.getCityList(id);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      apiResponse.response!.data.forEach((element) {
        CountryModel model = CountryModel.fromJson(element);
